...
if __name__ == "__main__":
    ...
    if not api_key:
        ...
        print("示例: export ZHIPU_API_KEY='your-api-key'")
        exit(1)

    try:
        # 创建检测器（使用智谱AI）
        detector = LLMDetectorAPI(provider='zhipu', api_key=api_key)

        # 显示检测器信息
        info = detector.get_info()
        print(f"\n提供商: {info['provider_name']}")
        print(f"模型: {info['model']}")
        print(f"API密钥: {'已设置' if info['has_api_key'] else '未设置'}")
